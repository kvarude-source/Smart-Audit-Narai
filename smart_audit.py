...
def get_base64_logo():
    ...
    p4 = '</svg>'
    return base64.b64encode((p1+p2+p3+p4).encode('utf-8')).decode("utf-8")
...
def apply_theme():
    ...
    <style>
        ...
        html, body, [class*="css"] { font-family: 'Prompt', sans-serif; color: #333333 !important; }
        section[data-testid="stSidebar"] { background-color: #FFFFFF !important; border-right: 1px solid #E0E0E0; }
        div.stButton > button { background-color: #1B5E20 !important; color: #FFFFFF !important; border: none; border-radius: 8px; padding: 12px 20px; font-weight: 600; width: 100%; box-shadow: 0 2px 5px rgba(0,0,0,0.2); }
        div.stButton > button:hover { background-color: #2E7D32 !important; }
        [data-testid="stDataFrame"] { background-color: #FFFFFF !important; border: 1px solid #E0E0E0; border-radius: 10px; padding: 5px; }
        [data-testid="stDataFrame"] div, [data-testid="stDataFrame"] span { color: #000000 !important; }
        [data-testid="stDataFrame"] th { background-color: #F1F8E9 !important; color: #1B5E20 !important; }
        .stTextInput input, .stPasswordInput input { background-color: #FFFFFF !important; color: #000000 !important; border: 1px solid #CCCCCC !important; border-radius: 6px; }
        .metric-card { background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border-left: 5px solid #1B5E20; text-align: center; }
        .login-box { background: white; padding: 40px; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); text-align: center; border-top: 5px solid #1B5E20; }
    </style>
...
def process_data_mock(uploaded_files):
    ...
    for i in range(150):
        ...
        if case == 'Over':
            ...
            row["FINDING"] = "-"; row["ACTION"] = "-"; row["IMPACT"] = 0
        data.append(row)
        
    df = pd.DataFrame(data)
    pre = 8500000.0
    imp = df['IMPACT'].sum()
    summ = {"records": 166196, "pre_audit": pre, "post_audit": pre + imp, "impact": imp}
    return df, summ

def get_ai_response(user_input):
    if not HAS_AI_CONNECTION: return f"{AI_ERROR_MSG}"
    try:
        info = "No Data"
        if st.session_state.summary:
            s = st.session_state.summary
            info = f"Records={s['records']:,}, Impact={s['impact']:,.0f}"
        prompt = f"Role: AI Consultant. Context: {info}. Question: {user_input}"
        
        m_name = 'gemini-pro'
        try:
            mods = genai.list_models()
            for m in mods:
                if 'flash' in m.name: m_name = m.name; break
        except: pass
...
def render_card(title, value, sub, is_impact=False):
    ...
    if is_impact:
        ...
        if val_num < 0: color = "#D32F2F"; sub = "▼ Overclaim"
        elif val_num > 0: color = "#388E3C"; sub = "▲ Underclaim"
...
def upload_page():
    ...
    st.markdown("---")
    st.markdown("""<div style="background:white; padding:40px; border:2px dashed #A5D6A7; text-align:center; margin-bottom:20px;"><h3 style="color:#1B5E20;">📂 Upload 52 Files</h3></div>""", unsafe_allow_html=True)
...
def dashboard_page():
    ...
    with c2: 
        ...
        if st.button("↺ Analyze New"):
            ...
            st.rerun()
    st.markdown("---")
    if st.session_state.audit_data is None: st.warning("Please upload data first."); return
    s = st.session_state.summary
    ...
    cfg = {"HN": st.column_config.TextColumn("HN", width="small"), "FINDING": st.column_config.TextColumn("Findings", width="large"), "ACTION": st.column_config.TextColumn("Action", width="large"), "IMPACT": st.column_config.NumberColumn("Impact", format="%.0f ฿")}
    def show(d):
        if not d.empty: st.dataframe(d, column_config=cfg, use_container_width=True, height=500)
...
def chat_page():
    ...
    if p := st.chat_input("Ask AI..."):
        ...
        with st.spinner("Thinking..."): ans = get_ai_response(p)
...
def main():
    ...
    if st.session_state.logged_in:
        with st.sidebar:
            ...
            st.write("")
            if st.button("💬 AI Consultant"): st.session_state.current_page = "chat"; st.rerun()
            st.write("")
            if st.button("📤 Upload Data"): st.session_state.current_page = "upload"; st.rerun()
            
            # --- Admin Log Button ---
            if st.session_state.username == "hosnarai":
                st.write(""); st.markdown("---"); st.markdown("**🔧 Admin Zone**")
                if st.button("📜 System Logs"): st.session_state.current_page = "logs"; st.rerun()
            # ------------------------
